...
// ── GENUS TAXONOMY & SPECIMEN FUNCTIONS ───────────────────────────────────────
...
function buildLineage(node: { parent: { name: string; parent: { name: string; parent: { name: string; parent: { name: string } | null } | null } | null } | null }): string[] {
  const path: string[] = []
  let current = node
  while (current.parent) {
    path.unshift(current.parent.name)
    current = current.parent as typeof current & { parent: typeof current.parent }
  }
  return path
}

function mapNode(n: { id: string; name: string; slug: string; level: string; specimenCount: number; description: string | null; children: Array<typeof n> }): TaxonomyNode {
  return {
    id: n.id,
    name: n.name,
    slug: n.slug,
...
export async function getSpecimenDetail(mbid: string): Promise<SpecimenDetail | null> {
  ...
  const familyClassification = classifications.find((c: { taxonomy: { level: string } }) => c.taxonomy.level === 'family')
  ...
  const deepest = [...classifications].sort((a: { taxonomy: { level: string } }, b: { taxonomy: { level: string } }) =>
  ...
  const relatedMbids = relatedRaw.map((r: { mbid: string }) => r.mbid)
  ...
  const relatedSpecimens: SpecimenSummary[] = relatedRaw.map((r: { mbid: string; name: string; country: string | null; type: string | null }) => ({
    mbid: r.mbid,
    name: r.name,
    country: r.country,
    type: r.type,
  ...
  return {
    ...
    primaryFamilySlug,
    lineage,
    tags: artist.tags.map((t: { tag: string; count: number }) => ({ tag: t.tag, count: t.count })),
    ...
    classifications: classifications.map((c: { taxonomyId: string; taxonomy: { name: string; slug: string; level: string } }) => ({
...
export async function searchSpecimens(query: string): Promise<SpecimenSummary[]> {
  ...
  const mbids = artists.map((a: { mbid: string }) => a.mbid)
  ...
  return artists.map((a: { mbid: string; name: string; country: string | null; type: string | null; imageUrl: string | null }) => ({
    mbid: a.mbid,
    name: a.name,
    country: a.country,
    type: a.type,
...
export async function getFeaturedSpecimens(): Promise<SpecimenSummary[]> {
  ...
  return rows.map((r: { mbid: string; name: string; country: string | null; type: string | null; imageUrl: string | null; famName: string; famSlug: string }) => ({
    mbid: r.mbid,
    name: r.name,
    country: r.country,
    type: r.type,
...
export async function getArtistHybridData(artistMbid: string): Promise<HybridData> {
  ...
  return {
    collaborators: collaborators.map((c: { mbid: string; name: string; count: bigint }) => ({ mbid: c.mbid, name: c.name, count: Number(c.count) })),
    samplesFrom,
    sampledBy,
  }
}

export async function getArtistSpotifyImage(mbid: string): Promise<string | null> {
  const artist = await prisma.artist.findUnique({
    where: { mbid },
    select: { name: true },
  }).catch(() => null)
  ...
  try {
    const results = await spotifyModule.searchSpotify(artist.name, 'artist', 1)
    const match = results.artists?.items?.[0]
